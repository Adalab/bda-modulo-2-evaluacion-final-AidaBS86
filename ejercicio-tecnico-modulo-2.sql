...
GROUP BY c.name;																			-- Y hago una agrupación por la categoria para que sea la base del count
...
WHERE f.film_id IN(												-- ...con el where in al film id.
	SELECT f.film_id, c.name, f.title							-- Hago primero la subconsulta unidendo las tablas necesarias y después la uno a la consulta principal...
	FROM film f
